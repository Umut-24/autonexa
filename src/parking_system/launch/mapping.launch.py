#!/usr/bin/env python3
"""
Minimal SLAM launch file for LiDAR-only robot (no encoders)
...
def generate_launch_description():
    ...
    slam_config = PathJoinSubstitution([pkg_dir, 'config', 'slam_toolbox_mapping.yaml'])
    ...
    # ============================================
    # NODE 3: SLAM TOOLBOX (MAPPER) - FRESH START
    # ============================================
    # Role: Builds NEW map from scratch (no old maps loaded)
    slam_toolbox = Node(
        ...
        parameters=[
            slam_config,
            {
                # FORCE FRESH START - Override any cached settings
                'mode': 'mapping',
                'map_file_name': '',           # EMPTY = no map loaded
