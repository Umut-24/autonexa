...
\begin{figure}[H]
    ...
    \node[block, below=of lidar] (perception) {Perception\\SLAM + Localization};
    \node[block, below=of perception] (nav2) {Navigation\\Nav2 Planner/Controller};
    \node[safety, below=of nav2] (safety) {Safety Chain\\Smoother + Collision Monitor};
    \node[io, below=of safety] (bridge) {ROS Bridge\\cmd\_vel\_to\_pico};
    \node[hw, below=of bridge] (pico) {Pico WH\\Real-Time Firmware};
    \node[hw, below=of pico] (act) {Actuation\\Servo + Motor};
    \node[io, right=26mm of pico] (health) {Health Feedback\\Heartbeat + Enable};

    \draw[line] (lidar) -- node[right]{\texttt{/scan}} (perception);
    \draw[line] (perception) -- node[right]{map, odom, tf} (nav2);
    \draw[line] (nav2) -- node[right]{\texttt{/cmd\_vel}} (safety);
    \draw[line] (safety) -- node[right]{\texttt{/cmd\_vel\_safe}} (bridge);
    \draw[line] (bridge) -- node[right]{\texttt{/pico/control\_cmd}} (pico);
    \draw[line] (pico) -- node[right]{PWM/I2C/GPIO} (act);
    \draw[line] (pico.east) -- (health.west);
    \draw[line] (health.north) |- node[pos=0.2,right]{\texttt{/pico/heartbeat}, \texttt{/pico/enable}} (bridge.east);
    \end{tikzpicture}
...
\textbf{Signal-level control flow (with interfaces):}
\begin{itemize}
    \item LiDAR $\rightarrow$ RPi5 perception: \texttt{/scan} (\texttt{sensor\_msgs/LaserScan}).
    \item Perception $\rightarrow$ Nav2: \texttt{/map}, \texttt{/odom}, \texttt{/tf} (localization and frame consistency).
    \item Nav2 $\rightarrow$ safety chain: \texttt{/cmd\_vel} (\texttt{geometry\_msgs/Twist} request command).
    \item Safety chain $\rightarrow$ bridge: \texttt{/cmd\_vel\_safe} (filtered command).
    \item RPi5 bridge $\rightarrow$ Pico via USB serial cable (micro-ROS): \texttt{/pico/control\_cmd} (bounded steering/speed command).
    \item Pico $\rightarrow$ RPi5 via USB serial cable (micro-ROS): \texttt{/pico/heartbeat}, \texttt{/pico/enable}, and status/feedback topics for watchdog supervision.
\end{itemize}
...
\subsection{Raspberry Pi 5 -- Pico Cable Signal Compatibility and Purpose}
The RPi5 and Pico communicate through a USB serial cable carrying micro-ROS traffic. Table~\ref{tab:rpi5_pico_signals} lists the key signals exchanged on this link and the operational purpose of each transmitted data channel.

\begin{table}[H]
\centering
\caption{Signal compatibility and purpose for RPi5 $\leftrightarrow$ Pico cable communication.}
\label{tab:rpi5_pico_signals}
\begin{tabularx}{\textwidth}{|p{2.2cm}|p{2.2cm}|p{3.1cm}|p{2.8cm}|X|}
\hline
\textbf{Direction} & \textbf{Layer} & \textbf{Signal / Topic} & \textbf{Data Type} & \textbf{Purpose} \\
\hline
RPi5 $\rightarrow$ Pico & micro-ROS over USB serial & \texttt{/pico/control\_cmd} & Control command message & Sends bounded steering/speed request from safety-filtered autonomy stack to firmware executor. \\
\hline
Pico $\rightarrow$ RPi5 & micro-ROS over USB serial & \texttt{/pico/heartbeat} & Heartbeat/status message & Confirms firmware liveness and communication continuity for watchdog supervision. \\
\hline
Pico $\rightarrow$ RPi5 & micro-ROS over USB serial & \texttt{/pico/enable} & Enable/state message & Indicates whether actuation path is enabled; used by bridge and supervisor for safe gating. \\
\hline
Pico $\rightarrow$ RPi5 & micro-ROS over USB serial & Encoder / telemetry topics & Feedback telemetry message(s) & Returns low-level execution feedback for monitoring, diagnostics, and closed-loop validation. \\
\hline
\end{tabularx}
\end{table}

\subsection{Exact Data Communication (Implemented Topics and Fields)}
The implemented RPi5$\leftrightarrow$Pico communication is topic-based over micro-ROS. For clarity, Table~\ref{tab:exact_fields_rpi5_pico} specifies the fields transmitted on the cable link. The same values can be logged or bridged in JSON for debugging and post-run analysis.

\begin{table}[H]
\centering
\caption{Exact transmitted fields for RPi5 $\leftrightarrow$ Pico communication.}
\label{tab:exact_fields_rpi5_pico}
\begin{tabularx}{\textwidth}{|p{2.7cm}|p{2.8cm}|p{3.2cm}|p{2.1cm}|X|}
\hline
\textbf{Topic} & \textbf{Direction} & \textbf{Field(s)} & \textbf{Unit} & \textbf{Purpose} \\
\hline
\texttt{/pico/control\_cmd} & RPi5 $\rightarrow$ Pico & \texttt{linear.x} & m/s & Target longitudinal speed command after safety filtering. \\
\cline{3-5}
 &  & \texttt{linear.y} & m/s & Reserved (set to 0 in Ackermann mode). \\
\cline{3-5}
 &  & \texttt{linear.z} & m/s & Reserved (set to 0). \\
\cline{3-5}
 &  & \texttt{angular.x} & rad/s & Reserved (set to 0). \\
\cline{3-5}
 &  & \texttt{angular.y} & rad/s & Reserved (set to 0). \\
\cline{3-5}
 &  & \texttt{angular.z} & rad/s & Yaw-rate/steering request used by Ackermann conversion. \\
\hline
\texttt{/pico/heartbeat} & Pico $\rightarrow$ RPi5 & \texttt{stamp.sec}, \texttt{stamp.nanosec} & s, ns & Liveness timestamp for watchdog supervision. \\
\cline{3-5}
 &  & \texttt{alive} & bool/int & Explicit heartbeat alive flag. \\
\hline
\texttt{/pico/enable} & Pico $\rightarrow$ RPi5 & \texttt{enabled} & bool & Indicates whether actuation is enabled in firmware. \\
\cline{3-5}
 &  & \texttt{fault\_code} & int & Reports firmware-side safety fault state (0 means nominal). \\
\hline
Encoder/telemetry topic(s) & Pico $\rightarrow$ RPi5 & \texttt{left\_ticks}, \texttt{right\_ticks} & ticks & Wheel feedback for odometry and motion validation. \\
\cline{3-5}
 &  & \texttt{wheel\_speed\_left}, \texttt{wheel\_speed\_right} & m/s or rad/s & Measured wheel speeds for closed-loop monitoring. \\
\hline
\end{tabularx}
\end{table}

\textbf{JSON examples for logging/bridge diagnostics:}
\begin{itemize}
    \item \textbf{RPi5 $\rightarrow$ Pico control command (\texttt{/pico/control\_cmd}):}
\begin{verbatim}
{
  "topic": "/pico/control_cmd",
  "msg_type": "geometry_msgs/Twist",
  "linear":  { "x": 0.22, "y": 0.0, "z": 0.0 },
  "angular": { "x": 0.0,  "y": 0.0, "z": 0.35 },
  "source": "cmd_vel_safe_bridge",
  "stamp": { "sec": 1711838400, "nanosec": 120000000 }
}
\end{verbatim}

    \item \textbf{Pico $\rightarrow$ RPi5 heartbeat (\texttt{/pico/heartbeat}):}
\begin{verbatim}
{
  "topic": "/pico/heartbeat",
  "alive": true,
  "loop_hz": 30.0,
  "stamp": { "sec": 1711838400, "nanosec": 150000000 }
}
\end{verbatim}

    \item \textbf{Pico $\rightarrow$ RPi5 enable status (\texttt{/pico/enable}):}
\begin{verbatim}
{
  "topic": "/pico/enable",
  "enabled": true,
  ...
  "stamp": { "sec": 1711838400, "nanosec": 150500000 }
}
\end{verbatim}

    \item \textbf{Pico $\rightarrow$ RPi5 encoder telemetry (example):}
\begin{verbatim}
{
  "topic": "/pico/encoder_feedback",
  "left_ticks": 15234,
  "right_ticks": 15198,
  "wheel_speed_left": 0.21,
  "wheel_speed_right": 0.20,
  "stamp": { "sec": 1711838400, "nanosec": 151000000 }
}
\end{verbatim}
\end{itemize}

Compatibility is ensured by message-type agreement (ROS2 contracts), shared timing assumptions (nominal command/update rates), and deterministic timeout behavior on both ends of the cable interface.
